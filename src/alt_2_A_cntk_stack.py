# https://www.kaggle.com/hoaphumanoid/data-science-bowl-2017/cntk-and-lightgbm-quick-start
...
def calc_features(verbose=True):
    ...
    net = get_extractor()
#    for folder in tqdm(glob.glob(STAGE1_FOLDER+'*')):
    all_patients=next(os.walk(STAGE1_FOLDER))[1]
    for folder in tqdm(all_patients):
        ...
        if os.path.isfile(os.path.join(FEATURE_FOLDER, foldername_npy)):
            ...
            continue
        batch = get_data_id(STAGE1_FOLDER+folder)
        if verbose:
            print("Batch size:")
            print(batch.shape)
        feats = batch_evaluation(net, batch, BATCH_SIZE)
#        ct_scan_id=foldername.split("/")[1]
        if verbose:
            print(feats.shape)
            print("Saving features in %s" % os.path.join(FEATURE_FOLDER, foldername_npy))
        np.save(os.path.join(FEATURE_FOLDER, foldername_npy), feats)
